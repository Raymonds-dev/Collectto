...
interface ProfileBackgroundCropModalProps {
  visible: boolean;
...
export function ProfileBackgroundCropModal({
  visible,
  imageUri,
  onCancel,
  onConfirm,
}: ProfileBackgroundCropModalProps) {
  const viewShotRef = useRef<ElementRef<typeof ViewShot> | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  const { width } = Dimensions.get('window');
  const cropWidth = useMemo(() => Math.min(width - 48, 360), [width]);
  const cropHeight = useMemo(() => Math.round(cropWidth * 0.42), [cropWidth]);
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    try {
      ...
      console.error('Erro ao recortar a imagem de capa:', error);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}>
      <View className="flex-1 items-center justify-center bg-overlay-scrim px-6">
        <View className="w-full max-w-md rounded-2xl bg-surface-card p-6">
          <Text className="font-poetsenone text-xl text-text-base">Ajuste sua capa</Text>
          <Text className="mt-2 text-sm text-text-muted">
            Arraste para reposicionar e use o zoom para enquadrar.
          </Text>

          <View className="mt-6 items-center justify-center">
            <View style={{ width: cropWidth, height: cropHeight }}>
              <ViewShot
                ref={viewShotRef}
                ...
                <GestureDetector gesture={gesture}>
                  <Animated.Image
                    source={{ uri: imageUri }}
                    style={[{ width: cropWidth, height: cropHeight }, imageStyle]}
                    resizeMode="cover"
                  />
                </GestureDetector>
              </ViewShot>
