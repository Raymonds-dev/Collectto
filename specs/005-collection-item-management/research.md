...
- Move without endpoint: `DEBUG-only` in first delivery.
- Delete without endpoint: plan assumes backend will expose DELETE endpoints for item/collection before API integration.
- `Sem categoria`: represented by a fixed system collection per user.

Operational implications

- Frontend service layer will expose unified commands (`update`, `delete`, `move`) and route to Debug adapter first.
- API adapter will implement `update` immediately and keep `move` as unsupported until backend provides endpoint.
- Delete contracts are defined now to avoid refactor when backend endpoints are published.
